...
A Java 21 library that parses Excel-compatible expression strings into typed Abstract Syntax Trees (ASTs) that can be evaluated, serialized, and updated through a data observer pattern.
...
if (result.isSuccess()) {
    ...
    // tree is a DoubleIfNode wrapping DoubleGreaterThanNode, DoubleMultiplyNode, DoubleAddNode
    
    // Evaluate with a DataProvider
    ...
    double value = typedTree.evaluate();
} else {
