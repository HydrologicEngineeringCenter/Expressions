...
public class Tokenizer {
    ...
    static {
        for (ExpressionOperator op : ExpressionOperator.values()) {
            ...
            if (infix != null && !infix.isEmpty() && !infix.contains("?") && !infix.contains("(") && !infix.contains(",") && !infix.contains(" ") && !infix.equals("[]")) {
        ...
        while (pos < len) {
            ...
            if (isNumberStart(c, input, pos)) {
                ...
                try {
                    ...
                    tokens.add(new Token.Unknown(numStr,pos, "Invalid number literal", remaining(input, pos)));
                    return tokens;
                }
                continue;
            }
            ...
            if (c == '[') {
                ...
                if (close == -1) {
                    tokens.add(new Token.Variable("[" + remaining(input,pos),pos, "Unclosed '[' for variable"));
                    return tokens;
                }
            ...
            if (Character.isLetter(c) || c == '_') {
                int end = pos;
                while (end < len && (Character.isLetterOrDigit(input.charAt(end)) || input.charAt(end) == '_')) {
                    end++;
                }
                String word = input.substring(pos, end);
                String upper = word.toUpperCase();

                if ("TRUE".equals(upper)) { tokens.add(new Token.StringLiteral("TRUE", pos,"")); pos = end; continue; }
                if ("FALSE".equals(upper)) { tokens.add(new Token.StringLiteral("FALSE", pos,"")); pos = end; continue; }
                ...
                tokens.add(new Token.Unknown(word, pos, "Unknown identifier: " + word, remaining(input, pos)));
                return tokens;
            }
            tokens.add(new Token.Unknown(String.valueOf(c), pos, "Unexpected character: '" + c + "'", remaining(input, pos)));
            return tokens;
        }
    ...
    private boolean isNumberStart(char c, String input, int pos) {
        ...
        return c == '.' && pos + 1 < input.length() && Character.isDigit(input.charAt(pos + 1));
    }
